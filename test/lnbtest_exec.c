...
START_TEST(test_execve)
{
	...
	fail("test_execve: the program didn't run, but it should: errno=%d\n", errno); /* should never be reached */
}
END_TEST
...
START_TEST(test_execveat)
{
	...
	if ( dirfd >= 0 )
	{
		...
		err = errno;
		close (dirfd);
		fail("test_execveat: the program didn't run, but it should: errno=%d\n", err); /* should never be reached */
	}
	else
	{
...
START_TEST(test_fexecve)
{
	...
	if ( prog_fd >= 0 )
	{
		...
		err = errno;
		close (prog_fd);
		fail("test_fexecve: the program didn't run, but it should: errno=%d\n", err); /* should never be reached */
	}
	else
	{
